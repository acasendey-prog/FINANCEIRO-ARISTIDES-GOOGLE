...
**Site:** https://financeiro-aristides.netlify.app (após o primeiro deploy)
...
## Consignados: como o saldo devedor é calculado

O sistema reproduz **a mesma conta que o Itaú faz** no *Documento Descritivo de
Crédito*. Cada parcela ainda em aberto é trazida a valor presente pela taxa do
contrato, descontada pela distância **real em dias** até o vencimento:

```
Quitar hoje  =  Σ  parcela ÷ (1 + i)^(dias até o vencimento ÷ 30)
```

Com a taxa oficial cadastrada, o número bate centavo a centavo com o extrato.
Conferência feita em 25/08/2026:

| Contrato   | Prazo | Parcela   | Taxa a.m. | Sistema      | Extrato Itaú |
|------------|-------|-----------|-----------|--------------|--------------|
| 3078321878 | 36×   | R$ 148,75 | 1,47 %    | R$ 3.978,63  | R$ 3.978,63  |
| 3120918747 | 120×  | R$ 76,56  | 1,54 %    | R$ 4.162,84  | R$ 4.162,82  |

### As colunas do demonstrativo

| Coluna | O que é |
|---|---|
| **Original** | o dinheiro que efetivamente caiu na conta (o IOF fica no valor financiado) |
| **Parc. / Parcela / Total** | prazo, valor fixo da parcela e o custo total se o contrato correr até o fim |
| **Taxa a.m.** | taxa mensal do contrato — a mesma do extrato |
| **Taxa a.a.** | taxa **efetiva** anual, (1 + a.m.)¹² − 1; maior que 12 × a mensal porque há juros sobre juros |
| **Pagas** | parcelas liquidadas; o selo `ant.` mostra quantas saíram por amortização extra |
| **Nominal** | soma bruta das parcelas que ainda faltam (parcela × parcelas em aberto). É **valor de face**: carrega os juros de todos os meses futuros, que ainda não correram. Responde "quanto ainda vai sair do bolso até o fim" — **nunca** é o preço de quitação |
| **Quitar hoje** | o preço real da liquidação antecipada hoje: as parcelas em aberto trazidas a valor presente |
| **Economia** | Nominal − Quitar hoje: os juros que deixam de correr se quitar agora |
| **Saldo devedor** | o mesmo valor presente, medido no mês de referência do sistema. É a série do gráfico de evolução |

### Amortização

A amortização de consignado **paga sempre as últimas parcelas do contrato, de
trás para a frente**. No formulário você informa *quantas* parcelas foram
abatidas (ou clica numa parcela na grade — dela até o fim entra no abatimento),
e o sistema:

1. mostra quais números de parcela foram atingidos (`31–36`);
2. sugere o valor presente dessas parcelas — você pode sobrescrever com o valor
   do comprovante, que é o que entra no fluxo de caixa;
3. cria o **lançamento de saída** no mês do pagamento, na categoria
   `Amortização <contrato>`;
4. recalcula na hora o demonstrativo, o resumo do contrato e o gráfico de
   evolução do saldo devedor.

Cada amortização guarda a lista exata de parcelas abatidas, então o histórico
continua auditável mesmo depois de vários pagamentos extras.
...
```jsonc
{
  ...
  "cartoes":      [{ "nome", "cat" }],
  "emprestimos":  [{ "cod", "contrato", "desc",
                     "principal",   // valor que caiu na conta
                     "financiado",  // principal + IOF — é sobre este valor que a taxa incide
                     "n", "parcela", "mes1",
                     "diaVenc",     // dia do desconto em folha (padrão 7)
                     "taxaMes",     // taxa mensal NOMINAL do contrato, do extrato do banco
                     "mesCredito",  // mês em que o dinheiro entrou
                     "amortizacoes": [{ "data", "valor", "parcelas": [], "lancId", "descricao" }] }]
}
```
...
O projeto **financeiro-aristides** já existe na Netlify, já está com a variável
...
```bash
npx -y netlify-cli deploy --prod --site financeiro-aristides
```

Ou, se preferir o fluxo por Git: suba os arquivos para um repositório no GitHub
e, em app.netlify.com → projeto **financeiro-aristides** → *Site configuration →
...
**Android (Chrome):**
1. Abra https://financeiro-aristides.netlify.app no Chrome.
...
1. Crie um repositório chamado `financeiro-aristides` (público ou privado).
2. Envie todos os arquivos deste pacote para a raiz do repositório.
3. **Settings → Pages → Source: Deploy from a branch → `main` / `(root)` → Save.**
4. Em 1–2 minutos o site fica no ar em
   `https://<seu-usuario>.github.io/financeiro-aristides/`.
...
```bash
git clone https://github.com/acasendey-prog/financeiro-aristides.git
cd financeiro-aristides
