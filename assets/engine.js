...
(function (global) {
  ...
  /* ----------------------------------------------------------- empréstimos */
  /* --------------------------------------------------------------------------
     Modelo de consignado (padrão Itaú / Price, parcela fixa em folha).

     Um contrato guarda:
       principal   valor líquido que caiu na conta
       financiado  principal + IOF (é sobre este valor que a taxa incide)
       n, parcela  prazo e valor fixo da parcela
       mes1        mês da 1ª parcela;  diaVenc  dia do desconto em folha
       taxaMes     taxa mensal NOMINAL do contrato (a do extrato). Sem ela,
                   o sistema deduz a taxa das próprias parcelas (TIR).
       amortizacoes[]  pagamentos extras, cada um com as parcelas que abateu

     Quitação antecipada segue a fórmula do banco: cada parcela em aberto é
     trazida a valor presente pela distância REAL em dias até o vencimento —
         VP = Σ parcela / (1 + i)^(dias/30)
     — o que reproduz o "Saldo Devedor" do Documento Descritivo de Crédito.
     -------------------------------------------------------------------------- */
  ...
  /** taxa mensal do contrato: a oficial do extrato quando houver, senão a TIR */
  E.taxaDoContrato = function (ep) {
    if (ep.taxaMes > 0) return ep.taxaMes;
    return E.taxaImplicita(ep.financiado || ep.principal, ep.parcela, ep.n);
  };
  E.diaVenc = function (ep) { return Math.min(28, Math.max(1, ep.diaVenc || 7)); };
  /** data de vencimento da parcela k (1-based), no formato AAAA-MM-DD */
  E.vencimento = function (ep, k) {
    var m = E.addMes(ep.mes1, k - 1);
    return m + '-' + String(Math.min(E.diaVenc(ep), E.diasNoMes(m))).padStart(2, '0');
  };
  E.diasEntre = function (a, b) {
    return Math.round((Date.parse(b + 'T12:00:00') - Date.parse(a + 'T12:00:00')) / 86400000);
  };
  /** valor presente de uma lista de nº de parcelas, na data `dataRef` */
  E.vpParcelas = function (ep, parcelas, dataRef, taxa) {
    var i = taxa === undefined ? E.taxaDoContrato(ep) : taxa, tot = 0;
    parcelas.forEach(function (k) {
      var dias = E.diasEntre(dataRef, E.vencimento(ep, k));
      if (dias < 0) dias = 0;
      tot += ep.parcela / Math.pow(1 + i, dias / 30);
    });
    return r2(tot);
  };
  /** data em que o saldo do mês `m` é medido: hoje, se for o mês corrente */
  E.dataRefDoMes = function (ep, m) {
    var hoje = E.hoje();
    if (m === hoje.slice(0, 7)) return hoje;
    return m + '-' + String(Math.min(E.diaVenc(ep), E.diasNoMes(m))).padStart(2, '0');
  };
  ...
  /** mapa completo de cada parcela: calendario | antecipada | aberta */
  E.mapaParcelas = function (db, ep, m) {
    m = m || db.meta.mesRef;
    var k = E.parcelasCalendario(ep, m), ant = E.parcelasAntecipadas(ep, m), out = [];
    for (var j = 1; j <= ep.n; j++) {
      out.push({
        n: j, mes: E.addMes(ep.mes1, j - 1), venc: E.vencimento(ep, j),
        estado: j <= k ? 'calendario' : (ant[j] ? 'antecipada' : 'aberta')
      });
    }
    return out;
  };
  /** as N últimas parcelas ainda em aberto — a amortização anda de trás para a frente */
  E.ultimasAbertas = function (db, ep, m, qtd) {
    var abertas = E.mapaParcelas(db, ep, m).filter(function (p) { return p.estado === 'aberta'; });
    return abertas.slice(Math.max(0, abertas.length - qtd)).map(function (p) { return p.n; });
  };

  /** situação completa do empréstimo em um mês de referência */
  E.emprestimo = function (db, ep, m) {
    m = m || db.meta.mesRef;
    var i = E.taxaDoContrato(ep);
    ...
    var nums = abertas.map(function (p) { return p.n; });

    var dataMes = E.dataRefDoMes(ep, m);           // saldo contábil no mês de referência
    var saldoMes = E.vpParcelas(ep, nums, dataMes, i);
    var hoje = E.hoje();
    var mapaHoje = E.mapaParcelas(db, ep, hoje.slice(0, 7));
    var numsHoje = mapaHoje.filter(function (p) { return p.estado === 'aberta'; }).map(function (p) { return p.n; });
    var quitar = E.vpParcelas(ep, numsHoje, hoje, i);

    var nominal = r2(ep.parcela * restantes);
    return {
      taxaMes: i,
      taxaAnoNominal: i * 12,
      taxaAno: Math.pow(1 + i, 12) - 1,
      oficial: ep.taxaMes > 0,
      financiado: ep.financiado || ep.principal,
      iof: r2((ep.financiado || ep.principal) - ep.principal),
      totalContrato: r2(ep.parcela * ep.n),
      custoTotal: r2(ep.parcela * ep.n - ep.principal),
      pagas: pagas, pagasCalendario: kCal, antecipadas: nAnt,
      restantes: restantes,
      nominalRestante: nominal,
      quitarHoje: quitar,
      saldoDevedor: saldoMes,
      economia: r2(nominal - saldoMes),
      dataRef: dataMes,
      primeiraAberta: restantes ? abertas[0].n : null,
      ultimaAberta: restantes ? abertas[restantes - 1].n : null,
      proximoVenc: restantes ? abertas[0].venc : null,
      ultima: E.addMes(ep.mes1, ep.n - 1),
      quitado: restantes <= 0,
      amortizado: E.amortizacoesAte(db, ep, m),
      parcelasAbatidas: nAnt,
      pctPago: ep.n ? pagas / ep.n : 0
    };
  };

  /** primeiro mês em que o contrato existe (mês do crédito, ou o anterior à 1ª parcela) */
  E.mesCredito = function (ep) { return ep.mesCredito || E.addMes(ep.mes1, -1); };

  /** horizonte do gráfico de dívida: vai até a última parcela do contrato mais longo */
  E.mesesDivida = function (db) {
    var a = db.meta.primeiroMes, z = db.meta.ultimoMes;
    db.emprestimos.forEach(function (ep) {
      var fim = E.addMes(ep.mes1, ep.n - 1);
      if (fim > z) z = fim;
    });
    var out = [], c = a, guard = 0;
    while (c <= z && guard++ < 900) { out.push(c); c = E.addMes(c, 1); }
    return out;
  };
  /** série do saldo devedor: uma linha por contrato + a linha do total */
  E.saldoDevedorPorEmprestimo = function (db) {
    var ms = E.mesesDivida(db);
    var linhas = db.emprestimos.map(function (ep) {
      var nasce = E.mesCredito(ep);
      return {
        cod: ep.cod, desc: ep.desc, contrato: ep.contrato || '',
  ...
  E.saldoDevedorSerie = function (db) {
    ...
    var lim = E.meses(db);
    return s.meses.filter(function (m) { return lim.indexOf(m) >= 0; }).map(function (m) {
      var idx = s.meses.indexOf(m);
      var parcelaMes = 0;
      db.emprestimos.forEach(function (ep) {
  ...
  /** consolidado da carteira de consignados no mês de referência */
  E.carteiraConsignado = function (db, m) {
    var acc = {
      contratos: db.emprestimos.length, ativos: 0, original: 0, financiado: 0,
      totalContrato: 0, nominal: 0, quitar: 0, saldo: 0, economia: 0,
      amortizado: 0, parcelaMes: 0, pagas: 0, antecipadas: 0, abertas: 0, parcelasTotais: 0
    };
    db.emprestimos.forEach(function (ep) {
      var x = E.emprestimo(db, ep, m);
      if (!x.quitado) { acc.ativos++; acc.parcelaMes += ep.parcela; }
      acc.original += ep.principal; acc.financiado += x.financiado;
      acc.totalContrato += x.totalContrato; acc.nominal += x.nominalRestante;
      acc.quitar += x.quitarHoje; acc.saldo += x.saldoDevedor; acc.economia += x.economia;
      acc.amortizado += x.amortizado; acc.pagas += x.pagas; acc.antecipadas += x.antecipadas;
      acc.abertas += x.restantes; acc.parcelasTotais += ep.n;
    });
    Object.keys(acc).forEach(function (k) { acc[k] = r2(acc[k]); });
    return acc;
  };
