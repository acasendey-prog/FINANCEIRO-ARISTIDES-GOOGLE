...
(function (global) {
  ...
  C.linha = function (host, o) {
    autoRender(host, function (w) {
      var h = o.altura || 250, pad = { t: 18, r: o.rotulos ? 34 : 16, b: 26, l: 58 };
      ...
      o.labels.forEach(function (l, i) {
        if (i % passo && i !== ult) return;
        if (i !== ult && Math.abs(X(ult) - X(i)) < 46) return;   // não encosta no último
        var t = el('text', {
          class: 'ax', x: X(i), y: h - 8,
          'text-anchor': i === ult ? 'end' : (i === 0 ? 'start' : 'middle')
        });
      ...
      /* ------------------------------------------------------------------
         Rótulos de dados. Um único mapa de ocupação para TODAS as séries:
         cada rótulo procura um lugar livre (acima, abaixo, mais acima…) e
         só é desenhado se couber. Assim nada se sobrepõe, mesmo com várias
         linhas cruzando no mesmo ponto.
         ------------------------------------------------------------------ */
      if (o.rotulos) {
        var caixas = [];
        var LARG = 46, ALT = 17;
        var cabe = function (x, y, txt) {
          if (y < pad.t + 6 || y > pad.t + ih + 4) return false;
          return !caixas.some(function (c) {
            if (c.txt === txt && Math.abs(c.x - x) < LARG * 1.8) return true;  // não repete o mesmo número
            return Math.abs(c.x - x) < LARG && Math.abs(c.y - y) < ALT;
          });
        };
        var passoR = Math.max(1, Math.ceil(o.labels.length / Math.max(2, Math.floor(iw / (o.labels.length > 60 ? 96 : 78)))));
        /* séries grossas (o total) rotulam primeiro e ganham o melhor lugar */
        var ordem = o.series.map(function (s, k) { return { s: s, k: k }; })
          .filter(function (z) { return z.s.rotular !== false; })
          .sort(function (a, b) { return (b.s.espessura || 2) - (a.s.espessura || 2); });

        ordem.forEach(function (z) {
          var s = z.s, zerouEm = -1;
          s.dados.forEach(function (v, i) {
            if (v === null || v === undefined) return;
            if (v === 0) { if (zerouEm >= 0) return; zerouEm = i; }   // só o 1º zero (quitação)
            var ultimo = i === o.labels.length - 1;
            if (i % passoR && !ultimo && i !== zerouEm) return;
            if (v === 0 && !o.rotularZero && i !== zerouEm) return;
            var x = X(i), base = Y(v), txt = curto(v);
            var tentativas = [base - 10, base + 17, base - 27];
            var y = null;
            for (var t = 0; t < tentativas.length; t++) {
              if (cabe(x, tentativas[t], txt)) { y = tentativas[t]; break; }
            }
            if (y === null) return;
            caixas.push({ x: x, y: y, txt: txt });
            var anchor = i === 0 ? 'start' : (ultimo ? 'end' : 'middle');
            var halo = el('text', { class: 'lbl-halo', x: x, y: y, 'text-anchor': anchor });
            halo.textContent = txt; svg.appendChild(halo);
            var t2 = el('text', { class: 'lbl-pt', x: x, y: y, 'text-anchor': anchor, fill: s.cor });
            t2.textContent = txt; svg.appendChild(t2);
          });
        });
      }
