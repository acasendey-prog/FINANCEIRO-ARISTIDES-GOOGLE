...
(function (global) {
  ...
  /* ------------------------------------------------------------------ marca */
  /** logotipo do sistema — tile em gradiente com barras ascendentes e seta */
  function logoSVG(tam) {
    var s = tam || 40, u = 'lg' + Math.random().toString(36).slice(2, 7);
    return '<svg viewBox="0 0 48 48" width="' + s + '" height="' + s + '" role="img" aria-label="Controle Financeiro">' +
      ...
      '<stop offset="0%" stop-color="#4ec6d6"/><stop offset="55%" stop-color="#2b93a5"/><stop offset="100%" stop-color="#14586a"/>' +
      '</linearGradient><clipPath id="' + u + 'c"><rect width="48" height="48" rx="12.5"/></clipPath></defs>' +
      '<rect width="48" height="48" rx="12.5" fill="url(#' + u + ')"/>' +
      '<g clip-path="url(#' + u + 'c)"><path d="M0 31 L48 8 L48 48 L0 48 Z" fill="#000" opacity=".10"/></g>' +
      '<rect x="9.4" y="28.8" width="6.8" height="10.2" rx="3.4" fill="#fff" fill-opacity=".42"/>' +
      '<rect x="20.6" y="22.4" width="6.8" height="16.6" rx="3.4" fill="#fff" fill-opacity=".66"/>' +
      '<path d="M35.2 9.2 L42.4 20.8 H38.6 V36.6 a2.4 2.4 0 0 1 -2.4 2.4 h-2 a2.4 2.4 0 0 1 -2.4 -2.4 V20.8 H28 Z" fill="#fff"/>' +
      '</svg>';
  }
  ...
  function sidebar() {
    ...
    return '<aside class="side" id="side">' +
      ...
      '<div><b>Controle Financeiro</b><span>Aristides</span></div></div>' +
  ...
  function viewCartoes(root) {
    ...
    /* o total só soma meses com fatura lançada — mês-base e meses sem fatura ficam de fora,
       senão o "à vista" negativo do mês sem extrato contaminaria a soma */
    ...
    root.innerHTML = topo('Cartões de crédito',
      ...
      '<b>cadastro de parcelamentos</b> — o sistema numera sozinho a 1ª até a última. ' +
      '<b>Fatura</b> é o que você efetivamente pagou: os lançamentos do mês na categoria do cartão, tirados do extrato. ' +
      '<b>À vista</b> é o que sobra: <b>fatura − parcelado</b>, ou seja, compras à vista, anuidade, juros e encargos daquela fatura. ' +
      'Meses sem fatura lançada aparecem como “—” e ficam fora dos totais; se houver parcela sem fatura, ' +
      'o cartão exibe um aviso para você lançar o pagamento.</div></div>' +
  ...
  function telaDestravar() {
    ...
    document.getElementById('app').innerHTML =
      ...
      '<div style="width:40px;height:40px;border-radius:11px;background:linear-gradient(145deg,#4ec6d6,#14586a);display:grid;place-items:center;color:#14071f;font-weight:700">SF</div>' +
